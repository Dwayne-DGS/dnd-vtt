...
const _listUsers = db.prepare("SELECT id, username, name, email, role, gm_requested, created_at, plan, trial_start, ai_cost, ai_credit, ai_period FROM users ORDER BY created_at");
...
const _setAiMeter = db.prepare("UPDATE users SET ai_cost = ?, ai_credit = ?, ai_used = ?, ai_period = ? WHERE id = ?");
const _addAiCredit = db.prepare("UPDATE users SET ai_credit = COALESCE(ai_credit,0) + ? WHERE id = ?");
...
export function setAiMeter(id, cost, credit, used, period) { _setAiMeter.run(cost, credit, used, period, id); }
export function addAiCredit(id, usd) { _addAiCredit.run(Number(usd) || 0, id); }
